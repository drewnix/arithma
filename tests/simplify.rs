...
mod test_simplify {
    ...
    fn test_simplify_like_terms() {
        let env = Environment::new();
        let expr = Node::Add(Box::new(Node::Variable("x".to_string())), Box::new(Node::Variable("x".to_string())));
        let simplified = Evaluator::simplify(&expr, &env).unwrap();
        assert_eq!(simplified, Node::Multiply(Box::new(Node::Number(2.0)), Box::new(Node::Variable("x".to_string()))));
    }

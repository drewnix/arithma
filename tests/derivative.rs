...
mod derivative_tests {
    use arithma::{build_expression_tree, differentiate_latex, Environment, Evaluator, Tokenizer};
    ...
    fn test_power_rule_derivatives() {
        ...
        assert!(
            ...
            "Derivative of x^0.5 at x=2 should be 0.25, got {}",
            result
        );
    }
    ...
    fn test_polynomial_derivatives() {
        ...
        let _expr = parse_expression("2*x^3 - 3*x^2 + x - 5").unwrap();
        ...
        let epsilon = 1e-10;
        assert!(
            (result - expected).abs() < epsilon || result == expected,
            "Derivative of 2x^3 - 3x^2 + x - 5 at x=2 should be 19, got {}",
            result
        );
    }
    ...
    fn test_quotient_rule() {
        ...
        assert_eq!(
            result, 1.5,
            "Derivative of (x^2 + 1)/(x - 1) at x=3 should be 1.5"
        );
    }
    ...
    fn test_chain_rule() {
        ...
        assert!(
            (result - expected).abs() < epsilon || result == expected,
            "Derivative of (2x + 1)^2 at x=1 should be 8, got {}",
            result
        );

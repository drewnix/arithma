...
**Strict input validation.** Every call's arguments are validated against the tool's published `inputSchema` before dispatch (`crates/mcp/src/validate.rs`): a type mismatch, an unknown field, or a missing required field is a JSON-RPC `-32602` error naming the offending key and the expected shape — never a silent ignore. The rationale: `args.get(key).and_then(as_type)` cannot distinguish "absent" from "present with the wrong type", so a downstream default silently substitutes and the tool answers a question the caller didn't ask. Objects that declare `properties` are closed — a typo'd field name is an error, not a silently inert option. An explicit `null` on an optional field means "not provided". The validator enforces the same schema object served by `tools/list`, so the advertised contract and the enforced one cannot drift.

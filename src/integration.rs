...
pub fn definite_integral(expr: &Node, var_name: &str, lower: f64, upper: f64) -> Result<f64, String> {
...
pub fn definite_integral_latex(latex_expr: &str, var_name: &str, lower: f64, upper: f64) -> Result<String, String> {
    // Parse the input expression
    let mut tokenizer = Tokenizer::new(latex_expr);
    let tokens = tokenizer.tokenize();
    let expr = build_expression_tree(tokens)?;
...
mod tests {
    ...
    fn test_latex_integration() {
        ...
        assert!(result.contains("+ C"), "Result should contain constant of integration");
        ...
        assert!(approx_eq(evaluated, 2.67, 0.01), 
